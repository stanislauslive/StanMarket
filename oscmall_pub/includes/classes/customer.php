...
//constructor
	...
	//start building array for adress book entries using the default address given on sign up  
		...
		if($customers_id == 0){
			...
			$this->set_default_address($this->address_id);
			...
			$L_customers_default_address_id = -1;
		    $L_qry = smn_db_query("SELECT customers_default_address_id FROM " . TABLE_CUSTOMERS . " WHERE customers_id= '" . (int)$customers_id . "'");
		    $L_defualt_address = smn_db_fetch_array($L_qry);
            $L_customers_default_address_id = $L_defualt_address ['customers_default_address_id'];
		    if ($L_customers_default_address_id != 1) {
			  smn_db_perform(TABLE_ADDRESS_BOOK, $sql_data_array, 'update', 'customers_id=' . (int)$customers_id . ' and address_book_id = ' . $L_customers_default_address_id);
		    }
			else{
	...
	function update_store_admin(){
		$sql_data_array = array(
		    'admin_firstname' => $this->firstname,
			'admin_lastname' => $this->lastname,
			'admin_email_address' => $this->email_address
		);
	...
	function set_state($state_name) {
		$this->state = smn_db_prepare_input(addslashes($state_name) );
	}
	...
	function set_tollfree($tollfree_number) {
		$this->tollfree = smn_db_prepare_input($tollfree_number);
	}
	...
	function set_default_address($address_id) {
		$this->default_address = (int)$address_id;
	}
	...
	function query($customer_id) {
		...
		if (smn_db_num_rows($qry1) > 0) {
		   ...
		   $this->address_id = $this->customer_data['customers_default_address_id'];
		   $this->firstname = $this->customer_data['customers_firstname'];
		   $this->lastname = $this->customer_data['customers_lastname'];
		   $this->email_address = $this->customer_data['customers_email_address'];
		   $this->telephone = $this->customer_data['customers_telephone'];
		   $this->tollfree = $this->customer_data['customers_tollfree'];
		   $this->fax = $this->customer_data['customers_fax'];
		   $this->newsletter = $this->customer_data['customers_newsletter'];
		   $this->gender = $this->customer_data['customers_gender'];
		   $this->dob = $this->customer_data['customers_dob'];

		   $this->password = $this->customer_data['customers_password'];
		   $qry2 = smn_db_query("SELECT * FROM " . TABLE_ADDRESS_BOOK . " WHERE address_book_id='" . $this->address_id . "'");
			if (smn_db_num_rows($qry2) > 0) {
				$this->address_data = smn_db_fetch_array($qry2);
		        $this->company = $this->address_data['entry_company'];
		        $this->street_address = $this->address_data['entry_street_address'];
		        $this->postcode = $this->address_data['entry_postcode'];
		        $this->city = $this->address_data['entry_city'];
		        $this->country_id = $this->address_data['entry_country_id'];
		        $this->gender = $this->address_data['entry_gender'];
		        $this->zone_id = $this->address_data['entry_zone_id'];
		        $this->state = $this->address_data['entry_state'];
			}else {
				$this->errno = 2;
			}
		}else {
