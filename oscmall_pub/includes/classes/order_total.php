...
// class constructor
    function order_total() {
      global $language, $store_id;
      if (defined('MODULE_ORDER_TOTAL_INSTALLED') && smn_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
        $this->modules = explode(';', MODULE_ORDER_TOTAL_INSTALLED);
        $prepare_module_string = str_replace('.php', '', MODULE_ORDER_TOTAL_INSTALLED);
    ...
    function credit_selection() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        $header_string .= '       <tr><td width="10">' .  smn_draw_separator('pixel_trans.gif', '10', '1') .'</td>' . "\n";
        $header_string .= '           <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n";
        $close_string   = '                           </table></td>';
        $close_string  .= '<td width="10">' .  smn_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
        $close_string  .= '</tr></table></td></tr></table></td>';
        $close_string  .= '<tr><td width="100%">' .  smn_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
        $this->modules = explode(';', MODULE_ORDER_TOTAL_INSTALLED);
        reset($this->modules);
        $output_string = '';
        ...
        while (list(, $value) = each($this->modules)) {
         ...
         //$use_credit_string = $GLOBALS[$class]->use_credit_amount();
            ...
            if ( ($use_credit_string !='' ) || ($selection_string != '') ) {
              $output_string .=  '<tr colspan="4"><td colspan="4" width="100%">' .  smn_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
              $output_string = ' <tr class="moduleRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" >' . "\n" .
                               '   <td width="10">' .  smn_draw_separator('pixel_trans.gif', '10', '1') .'</td>' .
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) {
            ...
            if ( ($use_credit_string !='' ) || ($selection_string != '') ) {
              ...
              $output_string = /*' <tr><td width="10">' .  smn_draw_separator('pixel_trans.gif', '10', '1') .'</td><td colspan=2><table border="0" cellpadding="2" cellspacing="0" width="100%"><tr class="moduleRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" >' . "\n" .
                               '   <td width="10">' .  smn_draw_separator('pixel_trans.gif', '10', '1') .'</td>' .*/
              $use_credit_string;
              $output_string .= '<td width="10">' . smn_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
      ...
      global $_POST,$HTTP_SESSION_VARS;
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ( ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) ) {
            ...
            if ($_POST[$post_var]) {
 	    if (!smn_session_is_registered($post_var)) smn_session_register($post_var);
...
//        if ($order->info['total'] - $total_deductions <= 0 ) {
	...
	if(!smn_session_is_registered('credit_covers')) smn_session_register('credit_covers');
          $credit_covers = true;
        }
    ...
    function clear_posts() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ( ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) ) {
            $post_var = 'c' . $GLOBALS[$class]->code;
	            if (smn_session_is_registered($post_var)) smn_session_unregister($post_var);
          }
        }
      }
    }
