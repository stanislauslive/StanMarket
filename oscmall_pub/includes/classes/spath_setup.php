...
//	function smn_get_store_path() {
	function get_store_path() {
		$sPath = '';
		
		$category_query = smn_db_query("select s2c.store_categories_id from " . TABLE_STORE_MAIN . " s, " . TABLE_STORE_TO_CATEGORIES . " s2c where s.store_id = '" . $this->store_id . "' and s.store_status = '1' and s.store_id = s2c.store_id limit 1");
...
// Return the number of products in a store_category
	...
	function  smn_set_sPath($sPath)	{
		...
		elseif (isset($this->store_id)) {
			return ($sPath = $this->smn_get_spath($this->store_id));
		} 
		else {
			return ($sPath = '1');
		}
