...
class store {
	...
	function store($store_id = 0) {
		...
		if (!$store_id == 0) {
			...
			if (smn_db_num_rows($qry_store) > 0) {
				...
				$Qcategory = smn_db_query('select store_categories_id from ' . TABLE_STORE_TO_CATEGORIES . ' where store_id = "' . $store_id . '"');
				$category = smn_db_fetch_array($Qcategory);
				...
				$this->set_store_category($category['store_categories_id']);
				$this->set_customers_id($customer_id);
			}
		}
	}

	function update_store_type() {
		$sql_array = array('customer_id' => $this->store_customers_id,
	...
	function create_store($use_store_id = 0) {
		...
		$sql_store_array = array('customer_id' => $customer_id,
	...
	function put_store_category() {
	  $Qcheck = smn_db_query('select count(*) as total from ' . TABLE_STORE_TO_CATEGORIES . ' where store_id = "' . $this->store_id . '"');
	  $check = smn_db_fetch_array($Qcheck);
	  if ($check['total'] > 0){
	    smn_db_query("update " . TABLE_STORE_TO_CATEGORIES . " set store_categories_id = '" . (int)$this->store_category . "' where store_id = '" . $this->store_id . "'");
	  }else{
	...
	function put_store_admin() {
		...
		$sql_data_array = array('admin_groups_id' => $this->store_type,
					...
					'store_id' => $this->store_id,
					'customer_id' => $this->store_customers_id,
	...
	function put_store_data() {
		global $store;
		$store_type_prefix = $store->smn_set_store_type($this->store_type);
		require(DIR_WS_FUNCTIONS . $store_type_prefix . '_store_sql.php');
		$newname = DIR_FS_CATALOG .'images/' . $this->store_id . '_images';
	...
	function put_store_products() {
		...
		$sql_data_array = array('store_id' => $this->store_id,
					...
					'customer_id' => (int)$this->store_customers_id, 
	...
	smn_mail($name, $this->store_customers_data['customers_email_address'], EMAIL_SUBJECT, $email_text, $store->get_store_owner(), $store->get_store_owner_email_address());
    	//email mall owner
	...
	function set_store_description($desc) {
		$this->store_description = addslashes($desc);
	}

	function set_customers_id($customer_id) {
		$this->store_customers_id = $customer_id;
		$qry = smn_db_query("SELECT * FROM " . TABLE_CUSTOMERS . " WHERE customers_id='" . (int)$customer_id . "'");
