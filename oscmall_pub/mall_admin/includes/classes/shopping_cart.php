...
// reset per-session cart contents, but not the database contents
      ...
      $products_query = smn_db_query("select products_id, customers_basket_quantity from " . TABLE_CUSTOMERS_BASKET . " where customers_id = '" . (int)$customer_id . "'");
    ...
    function get_store_list() {
      ...
      if (smn_session_is_registered('customer_id')){
        ...
        while ($store_products = smn_db_fetch_array($products_query)) {
          $this->store_list[] = array('store_id' => $store_products['store_id']);
        }
      }elseif (is_array($this->contents)) {
        reset($this->contents);
        while (list($products_id, ) = each($this->contents)) {
          if(!in_array($this->contents[$products_id]['store_id'], $this->store_list))
          $this->store_list[] = array('store_id' => $this->contents[$products_id]['store_id']);
        } 
      }
