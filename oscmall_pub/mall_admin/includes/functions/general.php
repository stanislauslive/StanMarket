...
// Parse the data used in the html tags to ensure the tags will not break
  ...
  function smn_output_string($string, $translate = false, $protected = false) {
    if ($protected == true) {
      ...
      if ($translate == false) {
        return smn_parse_input_field_data($string, array('"' => '&quot;'));
      } else {
        return smn_parse_input_field_data($string, $translate);
      }
    }
  }
  ...
  function smn_customers_name($customers_id) {
    $customers = smn_db_query("select customers_firstname, customers_lastname from " . TABLE_CUSTOMERS . " where customers_id = '" . (int)$customers_id . "'");
    $customers_values = smn_db_fetch_array($customers);
  ...
  function smn_get_path($current_category_id = '') {
    ...
    if ($current_category_id == '') {
      ...
      if (sizeof($cPath_array) == 0) {
        ...
        $cPath_new = '';
        $last_category_query = smn_db_query("select parent_id from " . TABLE_CATEGORIES . " where categories_id = '" . (int)$cPath_array[(sizeof($cPath_array)-1)] . "' and store_id = '". $store_id . "'");
        $last_category = smn_db_fetch_array($last_category_query);

        $current_category_query = smn_db_query("select parent_id from " . TABLE_CATEGORIES . " where categories_id = '" . (int)$current_category_id . "' and store_id = '". $store_id . "'");
  ...
  function smn_get_all_get_params($exclude_array = '') {
    ...
    while (list($key, $value) = each($_GET)) {
      if (($key != smn_session_name()) && ($key != 'error') && (!in_array($key, $exclude_array))) $get_url .= $key . '=' . $value . '&';
    }
  ...
  function smn_get_all_post_params($exclude_array = '') {
    ...
    while (list($key, $value) = each($_POST)) {
      if (($key != smn_session_name()) && ($key != 'error') && (!in_array($key, $exclude_array))) $post_url .= $key . '=' . $value . '&';
    }
...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function smn_datetime_short($raw_datetime) {
  ...
  function smn_draw_products_pull_down($name, $parameters = '', $exclude = '') {
    global $currencies, $languages_id, $store_id;
    ...
    $products_query = smn_db_query("select p.products_id, pd.products_name, p.products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.store_id = '". $store_id . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by products_name");
  ...
  function smn_options_name($options_id) {
    global $languages_id, $store_id;

    $options = smn_db_query("select products_options_name from " . TABLE_PRODUCTS_OPTIONS . " where products_options_id = '" . (int)$options_id . "' and language_id = '" . (int)$languages_id . "' and store_id = '" . $store_id . "' ");
    $options_values = smn_db_fetch_array($options);

    return $options_values['products_options_name'];
  }

  function smn_values_name($values_id) {
    global $languages_id, $store_id;

    $values = smn_db_query("select products_options_values_name from " . TABLE_PRODUCTS_OPTIONS_VALUES . " where products_options_values_id = '" . (int)$values_id . "' and language_id = '" . (int)$languages_id . "' and store_id = '" . $store_id . "' ");
  ...
  function smn_info_image($image, $alt, $width = '', $height = '') {
    if (smn_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES . $image)) ) {
      $image = smn_image(DIR_WS_CATALOG_IMAGES . $image, $alt, $width, $height);
    } else {
  ...
  function smn_get_country_name($country_id) {
    $country_query = smn_db_query("select countries_name from " . TABLE_COUNTRIES . " where countries_id = '" . (int)$country_id . "'");

    if (!smn_db_num_rows($country_query)) {
      return $country_id;
    } else {
  ...
  function smn_get_geo_zone_name($geo_zone_id) {
    global $store_id;
    
    $zones_query = smn_db_query("select geo_zone_name from " . TABLE_GEO_ZONES . " where geo_zone_id = '" . (int)$geo_zone_id . "' and store_id = '" . $store_id . "'");

    if (!smn_db_num_rows($zones_query)) {
      $geo_zone_name = $geo_zone_id;
    } else {
      $zones = smn_db_fetch_array($zones_query);
...
// Text Mode
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  function smn_get_zone_code($country, $zone, $def_state) {

    $state_prov_query = smn_db_query("select zone_code from " . TABLE_ZONES . " where zone_country_id = '" . (int)$country . "' and zone_id = '" . (int)$zone . "'");

    if (!smn_db_num_rows($state_prov_query)) {
      $state_prov_code = $def_state;
    }
    else {
      $state_prov_values = smn_db_fetch_array($state_prov_query);
  ...
  function smn_get_orders_status_name($orders_status_id, $language_id = '') {
    global $languages_id;

    if (!$language_id) $language_id = $languages_id;
    $orders_status_query = smn_db_query("select orders_status_name from " . TABLE_ORDERS_STATUS . " where orders_status_id = '" . (int)$orders_status_id . "' and language_id = '" . (int)$language_id . "'");
    $orders_status = smn_db_fetch_array($orders_status_query);

    return $orders_status['orders_status_name'];
  }

  function smn_get_orders_status() {
    global $languages_id;

    $orders_status_array = array();
    $orders_status_query = smn_db_query("select orders_status_id, orders_status_name from " . TABLE_ORDERS_STATUS . " where language_id = '" . (int)$languages_id . "' order by orders_status_id");
  ...
  function smn_get_products_name($product_id, $language_id = 0) {
    global $languages_id;

    if ($language_id == 0) $language_id = $languages_id;
    $product_query = smn_db_query("select products_name from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
...
// TABLES: manufacturers_info
  function smn_get_manufacturer_url($manufacturer_id, $language_id) {
    $manufacturer_query = smn_db_query("select manufacturers_url from " . TABLE_MANUFACTURERS_INFO . " where manufacturers_id = '" . (int)$manufacturer_id . "' and languages_id = '" . (int)$language_id . "'");
...
// This function is not available in all PHP versions so we test it before using it.
  function smn_class_exists($class_name) {
...
// TABLES: countries
  function smn_get_countries($default = '') {
...
// Alias function for Store configuration values in the Administration Tool
  ...
  function smn_get_store_types() {
    $store_types_array = array();
    $stores_query = smn_db_query("select store_types_id, store_types_name from " . TABLE_STORE_TYPES);
    while ($store_types = smn_db_fetch_array($stores_query)) {
      if($store_types['store_types_id'] != '1'){
      $store_types_array[] = array('id' => $store_types['store_types_id'],
                             'text' => $store_types['store_types_name']);
      }
    }
    return $store_types_array;
  }
...
// Sets the status of a banner
  function smn_set_banner_status($banners_id, $status) {
    if ($status == '1') {
      return smn_db_query("update " . TABLE_BANNERS . " set status = '1', expires_impressions = NULL, expires_date = NULL, date_status_change = NULL where banners_id = '" . $banners_id . "'");
    } elseif ($status == '0') {
      return smn_db_query("update " . TABLE_BANNERS . " set status = '0', date_status_change = now() where banners_id = '" . $banners_id . "'");
    } else {
      return -1;
    }
  }
...
// Sets the status of a product on special
  function smn_set_specials_status($specials_id, $status) {
    if ($status == '1') {
      return smn_db_query("update " . TABLE_SPECIALS . " set status = '1', expires_date = NULL, date_status_change = NULL where specials_id = '" . (int)$specials_id . "'");
    } elseif ($status == '0') {
      return smn_db_query("update " . TABLE_SPECIALS . " set status = '0', date_status_change = now() where specials_id = '" . (int)$specials_id . "'");
    } else {
      return -1;
    }
  }
...
// Retreive server information
  function smn_get_system_information() {
    ...
    return array('date' => smn_datetime_short(date('Y-m-d H:i:s')),
                 'system' => $system,
                 'kernel' => $kernel,
                 'host' => $host,
                 ...
                 'db_date' => smn_datetime_short($db['datetime']));
  }
  ...
  function smn_get_file_permissions($mode) {
// determine type
...
// adjust for SUID, SGID and sticky bit
  ...
  function smn_remove($source) {
    global $messageStack, $smn_remove_error;

    if (isset($smn_remove_error)) $smn_remove_error = false;
      ...
      while ($file = $dir->read()) {
        if ( ($file != '.') && ($file != '..') ) {
          if (is_writeable($source . '/' . $file)) {
            smn_remove($source . '/' . $file);
          } else {
            $messageStack->add(sprintf(ERROR_FILE_NOT_REMOVEABLE, $source . '/' . $file), 'error');
            $smn_remove_error = true;
          }
        }
      }
      $dir->close();
...
// Output the tax percentage with optional padded decimals
  ...
  function smn_get_tax_class_title($tax_class_id) {
    global $store_id;
    
    if ($tax_class_id == '0') {
      return TEXT_NONE;
    } else {
      $classes_query = smn_db_query("select tax_class_title from " . TABLE_TAX_CLASS . " where tax_class_id = '" . (int)$tax_class_id . "' and store_id = '" . $store_id . "'");
  ...
  function smn_banner_image_extension() {
...
// TABLES: tax_rates, zones_to_geo_zones
  function smn_get_tax_rate($class_id, $country_id = -1, $zone_id = -1, $use_store_id = '') {
    ...
    if ($use_store_id == ''){$use_store_id = $store_id;}
...
// TABLES: tax_rates, zones_to_geo_zones
  function smn_get_affiliate_tax_rate($class_id, $country_id, $zone_id) {
    ...
    $tax_query = smn_db_query("select SUM(tax_rate) as tax_rate from " . TABLE_TAX_RATES . " tr left join " . TABLE_ZONES_TO_GEO_ZONES . " za ON tr.tax_zone_id = za.geo_zone_id left join " . TABLE_GEO_ZONES . " tz ON tz.geo_zone_id = tr.tax_zone_id WHERE (za.zone_country_id IS NULL OR za.zone_country_id = '0' OR za.zone_country_id = '" . $country_id . "') AND (za.zone_id IS NULL OR za.zone_id = '0' OR za.zone_id = '" . $zone_id . "') AND tr.tax_class_id = '" . $class_id . "' and tr.store_id = ' " . $store_id . "' and za.store_id = ' " . $store_id . "' GROUP BY tr.tax_priority");
    if (smn_db_num_rows($tax_query)) {
      $tax_multiplier = 0;
      while ($tax = smn_db_fetch_array($tax_query)) {
        $tax_multiplier += $tax['tax_rate'];
...
// Output a day/month/year dropdown selector
  function smn_draw_date_selector($prefix, $date='') {
    $month_array = array();
...
// Credit class gift voucher begin
////
// Sets the status of a coupon
  function smn_set_coupon_status($coupon_id, $status) {
    if ($status == 'Y') {
      return smn_db_query("update " . TABLE_COUPONS . " set coupon_active = 'Y', date_modified = now() where coupon_id = '" . (int)$coupon_id . "'");
    } elseif ($status == 'N') {
      return smn_db_query("update " . TABLE_COUPONS . " set coupon_active = 'N', date_modified = now() where coupon_id = '" . (int)$coupon_id . "'");
    } else {
      return -1;
    }
  }
// Credit class gift voucher end
