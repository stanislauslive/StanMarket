...
*/
  ...
  if (smn_not_null($action)) {
    switch ($action) {
      ...
      case 'insert':
      case 'update':
        if (isset($HTTP_POST_VARS['newsletter_id'])) $newsletter_id = smn_db_prepare_input($HTTP_POST_VARS['newsletter_id']);
        $newsletter_module = smn_db_prepare_input($HTTP_POST_VARS['module']);
        $title = smn_db_prepare_input($HTTP_POST_VARS['title']);
        $content = smn_db_prepare_input($HTTP_POST_VARS['content']);
...
<script language="Javascript1.2" src="<?php echo HTTP_SERVER . DIR_WS_ADMIN; ?>editor.js"></script>
<script>
...
_editor_url = "<?php echo HTTP_SERVER. DIR_WS_ADMIN;  ?>";
</script>
<style type="text/css"><!--
...
?>
      ...
      <tr><?php echo smn_draw_form('newsletter', FILENAME_NEWSLETTERS, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=' . $form_action); if ($form_action == 'update') echo smn_draw_hidden_field('newsletter_id', $nID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            <td td colspan="2 class="main" valign="top"><?php echo TEXT_NEWSLETTER_CONTENT; ?></td>
          </tr>
          <tr>
<td colspan="2>            
<?php

include(DIR_FS_ADMIN . 'fckeditor.php') ;
// Automatically calculates the editor base path based on the _samples directory.
// This is usefull only for these samples. A real application should use something like this:
$oFCKeditor->BasePath = DIR_FS_ADMIN;
// FCKeditoris the default value.

$oFCKeditor = new FCKeditor('content') ;

$oFCKeditor->Value = $nInfo->content;
$oFCKeditor->Create() ;
?>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main" align="right"><?php echo (($form_action == 'insert') ? smn_image_submit('button_save.gif', IMAGE_SAVE) : smn_image_submit('button_update.gif', IMAGE_UPDATE)). '&nbsp;&nbsp;<a href="' . smn_href_link(FILENAME_NEWSLETTERS, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . (isset($HTTP_GET_VARS['nID']) ? 'nID=' . $HTTP_GET_VARS['nID'] : '')) . '">' . smn_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
          </tr>
        </table></td>
      </form></tr>
<?php
...
?>
      ...
      <tr>
        <td align="right"><?php echo '<a href="' . smn_href_link(FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $HTTP_GET_VARS['nID'] . '&action=new') . '">' . smn_image_button('button_edit.gif', IMAGE_EDIT) . '</a>&nbsp;<a href="' . smn_href_link(FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $HTTP_GET_VARS['nID']) . '">' . smn_image_button('button_back.gif', IMAGE_BACK) . '</a>';  ?></td>
      </tr>
<?php
  } elseif ($action == 'send') {
    $nID = smn_db_prepare_input($HTTP_GET_VARS['nID']);
...
?>
      ...
      <tr>
        <td><?php echo '<a href="' . smn_href_link(FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $HTTP_GET_VARS['nID']) . '">' . smn_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
      </tr>
<?php
  } else {
?>
...
<?php
    ...
    if ((!isset($HTTP_GET_VARS['nID']) || (isset($HTTP_GET_VARS['nID']) && ($HTTP_GET_VARS['nID'] == $newsletters['newsletters_id']))) && !isset($nInfo) && (substr($action, 0, 3) != 'new')) {
      ...
      if (isset($nInfo) && is_object($nInfo) && ($newsletters['newsletters_id'] == $nInfo->newsletters_id) ) {
        echo '                  <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . smn_href_link(FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->newsletters_id . '&action=preview') . '\'">' . "\n";
      } else {
        echo '                  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . smn_href_link(FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $newsletters['newsletters_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo '<a href="' . smn_href_link(FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $newsletters['newsletters_id'] . '&action=preview') . '">' . smn_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $newsletters['title']; ?></td>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($nInfo) && is_object($nInfo) && ($newsletters['newsletters_id'] == $nInfo->newsletters_id) ) { echo smn_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . smn_href_link(FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $newsletters['newsletters_id']) . '">' . smn_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
